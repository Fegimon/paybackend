...
$listcounts=$conn->getRow("Select * from (select count(id) as pencount from ".PRODUCTRETURN." WHERE status='W' and user_id='". $_SESSION['prentz_user_id']."' limit 1)as p, (select count(id) as procount from ".PRODUCTRETURN." WHERE status='Y' and user_id='". $_SESSION['prentz_user_id']."'  limit 1)as pr, (select count(id) as comcount from ".PRODUCTRETURN." WHERE status='C' and user_id='". $_SESSION['prentz_user_id']."' limit 1)as shp, (select count(id) as cancount from ".PRODUCTRETURN." WHERE status='N' and user_id='". $_SESSION['prentz_user_id']."' limit 1)as can");

$comments = $conn->select_query(PRODUCTRETURN,"*",$cond."order by id desc","");
...
<button type="button" class="btn btn-info btn-service" data-toggle="modal" data-target="#addproduct">
Product Return
</button>
</div>-->
...
<div class="pull-right pull-none">
<div class="btn-group">
<a href="<?php echo SITE_URL; ?>product-return.php?status=pending" class="btn btn-dash btn-info pending-btn" role="button">Pending (<?php echo  $listcounts['pencount'];?>)</a>
</div>
<div class="btn-group">
<a href="<?php echo SITE_URL; ?>product-return.php?status=processing" class="btn btn-dash btn-info processing-btn" role="button">Processing (<?php echo  $listcounts['procount'];?>)</a>
</div>
<div class="btn-group">
<a href="<?php echo SITE_URL; ?>product-return.php?status=completed" class="btn btn-dash btn-info complete-btn" role="button">Complete (<?php echo  $listcounts['comcount'];?>)</a>
</div>
<div class="btn-group">
<a href="<?php echo SITE_URL; ?>product-return.php?status=cancelled" class="btn btn-dash btn-info cancelled-btn" role="button">Cancelled(<?php echo  $listcounts['cancount'];?>)</a>
</div>
...
<div class="modal fade" id="addproduct" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h3 class="modal-title" id="exampleModalLongTitle">Product Return <button type="button" class="close" data-dismiss="modal" aria-label="Close">
 ...
 <div class="form-group">
   ...
   <select name="pro_id" id="pro_id" class="form-control" id="sel1">
 ...
 <div class="form-group">
     <label for="">Date & Time</label>
      <div class="input-group date-time" id="datetimepicker">
          <input name="datetime" id="datepicker" class="form-control"/><span class="input-group-addon"><span class="fa fa-calendar"></span></span>
        </div>
</div>
  <div class="form-group">
    ...
    <textarea  name="comments" id="comments" class="form-control" required rows="4"></textarea>
      ...
      <div class="modal-footer">
       <button type="submit" name="pro_save" class="btn btn-primary btn-red">Submit</button>
      </div>
