...
//$conn->valoperator("2");
$listcounts=$conn->getRow("Select * from (select count(id) as pencount from ".SERVICEREQUEST." WHERE status='W' limit 1)as p, (select count(id) as procount from ".SERVICEREQUEST." WHERE status='Y' limit 1)as pr, (select count(id) as comcount from ".SERVICEREQUEST." WHERE status='C' limit 1)as shp, (select count(id) as cancount from ".SERVICEREQUEST." WHERE status='N' limit 1)as can");
 ?>
<div class="row">
