...
$fromdate=date('Y-m-d',strtotime($_REQUEST['dt_from']));
  ...
  $cond ="";
    ...
    if($fromdate)
    {
      $cond .= "serdate>='".$fromdate."'";
    }
    if($todate)
    {
      $cond .= "AND serdate<='".$todate."'";
    }
	if($status!='all')
	{
...
$orders=$conn->select_query(SERVICEREQUEST,"*","$cond order by id desc",""); 
...
$objPHPExcel->getActiveSheet()->SetCellValue('H2', 'Service Request Details');
...
foreach ($xl as $key => $value) 
{
  ...
  if($value['serdate']!='0000-00-00')
  {
	  $serdate=date('d-m-Y',strtotime($value['serdate']));
  }else
  {
	  $serdate='Nill';
  }
...
$objPHPExcel->getActiveSheet()->SetCellValue('F'.$i, $ttxt);
$objPHPExcel->getActiveSheet()->SetCellValue('G'.$i, $serdate);

	$i++;
	$sno++;
}
$objPHPExcel->getActiveSheet()->setTitle('Service Request Report');
$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
header('Content-type: application/vnd.ms-excel');
header('Content-Disposition: attachment; filename="Service Request Report.xls"');
