...
if($btn_action=="Action"&&$action=="processing")
{
  ...
  //print_r($selData);
	...
	$upd_processing = $conn->Execute("UPDATE ".SERVICEREQUEST." SET status='Y' WHERE id IN(".$selData.")");
	if($upd_processing)
	{
...
if($btn_action=="Action"&&$action=="completed")
{
  ...
  $upd_completed = $conn->Execute("UPDATE ".SERVICEREQUEST." SET status='C' WHERE id IN(".$selData.")");
  if($upd_completed)
  {
...
if($btn_action=="Action"&&$action=="cancelled")
{
  ...
  $upd_cancelled = $conn->Execute("UPDATE ".SERVICEREQUEST." SET status='N' WHERE id IN(".$selData.")");
  if($upd_cancelled)
  {
...
{
	$selData = @implode(",",$chkall);
	$upd_inactive = $conn->Execute("UPDATE ".SERVICEREQUEST." SET status='D' WHERE id IN(".$selData.")",1);
...
$selTable = $conn->pagination(SERVICEREQUEST,"*",$cond,"id desc",$website,$paging,$_GET['page'],1);
 ...
 $orders = $conn->select_query(ORDER,"*","ord_id ='".$res['main_ord_id']."'","1");
  ...
  //  $order_status = $conn->select_query(SERVICEREQUEST,"*","status='".$res['status']."' AND status='W'","1");
                  ...
                  <tr class="tbldatarow">
                    ...
                    <td class="td_text"><?php echo $res['product_name']; ?></td>
