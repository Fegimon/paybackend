...
if($btn_action=="Action"&&$action=="Inactive")
{
	...
	$upd_inactive = $conn->Execute("UPDATE ".LOCATION." SET lo_status='N' WHERE lo_id IN(".$selData.")");
	if($upd_inactive)
	{
...
if($btn_action=="Action"&&$action=="Active")
{
	...
	$upd_inactive = $conn->Execute("UPDATE ".LOCATION." SET lo_status='Y' WHERE lo_id IN(".$selData.")");
	if($upd_inactive)
	{
...
{
	$selData = @implode(",",$chkall);
	$upd_inactive = $conn->Execute("UPDATE ".LOCATION." SET lo_status='D' WHERE lo_id IN(".$selData.")",1);
	if($upd_inactive)
	{
...
if($txtsearch)
{
	$cond .= " AND (lo_name LIKE '%".base64_decode($txtsearch)."%')";
...
$selTable = $conn->pagination(LOCATION,"*",$cond,"",$website,$paging,$_GET['page']);
?>
<?php #Admin Html head
$conn->adminHtmlhead();
$conn->admninBody();
              ...
              <div class="box-body table-responsive no-padding">
                ...
                <table class="table table-hover">
                  <tr class="bg-gray color-palette">
                    ...
                    <th class="head_text" width="10%">Location</th>
                  ...
                  <tr class="tbldatarow">
					...
					<td class="td_text"><?php echo $conn->stripval(ucfirst($res['lo_name']));?></td>													
					<td class="td_text"><?php echo date("d-m-Y",strtotime($res['lo_date_dt']));?></td>
					<td class="td_text"><a class="btn btn-default" href="<?php echo $path_folder."form.php"?>?q=<?php echo $res['lo_id'].'&rpage='.base64_encode($conn->getOwnURL());?>"  title="Edit"><i class="fa fa-edit"></i></a></td>					
				  <td class="td_text"><input name="chkall[]" id="chkall" class="chkall" type="checkbox" value="<?php echo $res['lo_id']; ?>" /></td>
                  </tr>
                  <?php }?>
                </table>
                <?php }else{?>
