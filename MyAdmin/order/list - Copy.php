...
if($btn_action=="Action"&&$action=="processing")
{
  ...
  //print_r($selData);
	...
	$upd_processing = $conn->Execute("UPDATE ".ORDERPRODUCT." SET order_status_id='2' WHERE order_id IN(".$selData.")");
	if($upd_processing)
	{
...
if($btn_action=="Action"&&$action=="completed")
{
  ...
  $upd_completed = $conn->Execute("UPDATE ".ORDERPRODUCT." SET order_status_id='5' WHERE order_id IN(".$selData.")");
  if($upd_completed)
  {
...
if($btn_action=="Action"&&$action=="shipped")
{
  ...
  $upd_shipped = $conn->Execute("UPDATE ".ORDERPRODUCT." SET order_status_id='3' WHERE order_id IN(".$selData.")");
  if($upd_shipped)
  {
...
if($btn_action=="Action"&&$action=="cancelled")
{
  ...
  $upd_cancelled = $conn->Execute("UPDATE ".ORDERPRODUCT." SET order_status_id='6' WHERE order_id IN(".$selData.")");
  if($upd_cancelled)
  {
...
$selTable = $conn->pagination(ORDERPRODUCT,"*",$cond,"order_id desc",$website,$paging,$_GET['page']);
?>
<?php #Admin Html head
$conn->adminHtmlhead();
$conn->admninBody();
        ...
        <div class="col-xs-12">
          ...
          <div class="box <?php echo ($token=="inactive")?"box-danger":"box-success"; ?>">
            ...
            <form method="post" name="frm_list" id="frm_list" >
              <div class="box-body table-responsive no-padding">
                ...
                <table class="table table-hover">
                  <tr class="bg-gray color-palette">
                    ...
                    <th class="head_text" width="25%">Customer </th>
                    <th class="head_text" width="15%">Status</th>
                ...
                $order_status = $conn->select_query(ORDERSTATUS,"*","order_status_id='".$res['order_status_id']."' AND status='Y'","1");
                  ...
                  <tr class="tbldatarow">
                    ...
                    <td class="td_text"><?php echo $conn->stripval($res['ord_id']);?></td>
                    <td class="td_text"><?php echo ucfirst($res['firstname']). ' '. $res['lastname']; ?></td>
                   ...
                   <?php /* ?> <td class="td_text"><a class="btn btn-default" href="<?php echo $path_folder."form.php"?>?q=<?php echo $res['tax_id'].'&rpage='.base64_encode($conn->getOwnURL());?>"  title="Edit"><i class="fa fa-edit"></i></a></td><?php */ ?>
					...
					<td class="td_text"><a class="btn btn-default" href="<?php echo $path_folder."view.php"?>?q=<?php echo $res['order_id'];?>"  title="View"><i class="fa fa-file-text"></i></a></td>
				<?php  ?><td class="td_text"><input name="chkall[]" id="chkall" class="chkall" type="checkbox" value="<?php echo $res['order_id']; ?>" /></td><?php  ?>
                  </tr>
                  <?php }?>
                </table>
                <?php }else{?>
              ...
              <?php if($selTable['nr'] && $token!='completed' && $token!='completed'){?>
