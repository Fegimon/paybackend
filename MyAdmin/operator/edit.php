...
if(isset($btn_sub))
{
	if($op_uname!=''&&$op_name!='')
	{
		$User = $conn->select_query(OPERATOR,"op_id","op_status!='D' AND op_uname='".$conn->variable($op_uname)."' AND op_id!='".$id."'","1");
		if(!$User['nr'])
		{
			$password=$conn->encode($_REQUEST['password']);
			
			$feat_id=@implode(",",$feat);
			$arr=array('op_password'=>$password,'feat_id'=>$feat_id);
			$ins = $conn->update(OPERATOR,"op_id='".$id."'",$arr);
			if($ins)
			{
		...
		{
			$errAlert = "Username aleady exist.";
		}
	}else
	{
          ...
          <div class="box box-primary">
            <div class="box-header">
              ...
              <div class="pull-right">
                ...
                <a style="margin-right:4px;" class="btn  btn-default btn-xs text-purple" href="<?php echo $rpage; ?>"><i class="fa fa-arrow-left"></i> Back</a> </div>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              <form role="form" method="post" name="frm_new" id="frm_new" action="" enctype="multipart/form-data">
                ...
                <div class="form-group">
                  ...
                  <input name="op_uname" id="op_uname" type="text" class="form-control validate[required,funcCall[checkoperator],custom[username]]" placeholder="Username" maxlength="50" value="<?php echo $sel['op_uname'];?>" />
                </div>
                <div class="form-group">
                  <label>Password <span class="text-red">*</span></label>
                  <input name="password" id="password" class="form-control validate[required,minSize[5]]" type="text" placeholder="Username" maxlength="30" value="<?php echo $conn->decpyt($sel['op_password']);?>"  />
                </div>
                <?php $oparr=@implode(',',$operator_array);	 ?>
                <div class="box-header">
                  <h3 class="box-title text-navy">Features</h3>
                  <div class="box-tools pull-right">
                    <label>
                      <input name="all" class="mall"  type="checkbox" value="<?php echo $res['banner_id']; ?>"  onclick="mark();" <?php if($oparr==$sel['feat_id']){ ?> checked="checked"<?php }?>  />
                      &nbsp;Select All</label>
                  </div>
                </div>
                <div class="form-group">
				...
				{ ?>
                  <div class="col-md-4" >
                    <div class="checkbox">
                      <label>
                        <input type="checkbox" name="feat[]" id="feat<?php echo $resmenu['menu_id']; ?>" class="opfeat validate[minCheckbox[1]]"  value="<?php echo $resmenu['menu_id']; ?>" <?php if(@in_array($resmenu['menu_id'],$fid)){ ?>checked="checked"<?php } ?> />
                        <?php echo $conn->stripval($resmenu['menu_title']); ?></label>
                    </div>
                  </div>
                  <?php }}?>
                </div>
