...
if($btn_action=="Action"&&$action=="Inactive")
{
	...
	$upd_inactive = $conn->Execute("UPDATE ".COMMENT." SET lt_status='N' WHERE lt_id IN(".$selData.")");
	if($upd_inactive)
	{
...
if($btn_action=="Action"&&$action=="Active")
{
	...
	$upd_inactive = $conn->Execute("UPDATE ".COMMENT." SET lt_status='Y' WHERE lt_id IN(".$selData.")");
	if($upd_inactive)
	{
...
{
	$selData = @implode(",",$chkall);
	$upd_inactive = $conn->Execute("UPDATE ".COMMENT." SET lt_status='D',lt_profile='' WHERE lt_id IN(".$selData.")",1);
	if($upd_inactive)
	{
		#DELETE IMAGE
		$delDat = $conn->select_query(COMMENT,"lt_profile","lt_id IN(".$selData.")","");
		if($delDat['nr'])
		{
			foreach($delDat['result']as $resdat)
			{
				$folder="../../".$uploadFolder;
				if($resdat['lt_profile']!="")
				{
					$name=$resdat['lt_profile'];
					@unlink($folder.$name);
				}
			}
		}
...
#Select Records
$cond = ($token=="inactive") ? "lt_status='N'" : "lt_status='Y'";
if($txtsearch)
{
	$cond .= " AND (lt_name LIKE '%".base64_decode($txtsearch)."%' OR lt_designation LIKE '%".base64_decode($txtsearch)."%' OR lt_desc LIKE '%".base64_decode($txtsearch)."%')";
...
$selTable = $conn->pagination(COMMENT,"*",$cond,"lt_date_dt desc",$website,$paging,$_GET['page']); 
?>
<?php #Admin Html head
$conn->adminHtmlhead();
$conn->admninBody();
              ...
              <div class="box-body table-responsive no-padding">
                ...
                <table class="table table-hover">
                  <tr class="bg-gray color-palette">
                    ...
                    <th class="head_text" width="15%">Designation</th>
                  ...
                  <?php 
							...
							{
								$sino++;
								$TMexist = $conn->image_exist($res['lt_profile'],"../../".$uploadFolder);
								$img = ($TMexist) ? $uploadFolder.$res['lt_profile'] : "images/noimg.jpg";
						   ?>
                  <tr class="tbldatarow">
                    <td class="td_text"><?php echo $sino; ?></td>
                    <td class="td_text"><?php echo $conn->stripval(ucfirst($res['lt_name']));?></td>
                    <td class="td_text"><?php echo $conn->stripval($res['lt_designation']);?></td>
                    ...
                    <td class="td_text"><a class="btn btn-default" href="<?php echo $path_folder."form.php"?>?q=<?php echo $res['lt_id'].'&rpage='.base64_encode($conn->getOwnURL());?>"  title="Edit"><i class="fa fa-edit"></i></a></td>
					<td class="td_text"><a class="btn btn-default" href="<?php echo $path_folder."view.php"?>?q=<?php echo $res['lt_id'];?>"  title="View"><i class="fa fa-file-text"></i></a></td>
				<td class="td_text"><input name="chkall[]" id="chkall" class="chkall" type="checkbox" value="<?php echo $res['lt_id']; ?>" /></td>
                  </tr>
                  <?php }?>
                </table>
                <?php }else{?>
