...
//$conn->valoperator("2");
$listcounts=$conn->getRow("Select * from (select count(id) as pencount from ".PRODUCTTRANSFER." WHERE status='W' limit 1)as p, (select count(id) as procount from ".PRODUCTTRANSFER." WHERE status='Y' limit 1)as pr, (select count(id) as comcount from ".PRODUCTTRANSFER." WHERE status='C' limit 1)as shp, (select count(id) as cancount from ".PRODUCTTRANSFER." WHERE status='N' limit 1)as can");
 ?>
<div class="row">
