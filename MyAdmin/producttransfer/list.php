...
if($btn_action=="Action"&&$action=="processing")
{
  ...
  //print_r($selData);
	...
	$upd_processing = $conn->Execute("UPDATE ".PRODUCTTRANSFER." SET status='Y' WHERE id IN(".$selData.")");
	if($upd_processing)
	{
...
if($btn_action=="Action"&&$action=="completed")
{
  ...
  $upd_completed = $conn->Execute("UPDATE ".PRODUCTTRANSFER." SET status='C' WHERE id IN(".$selData.")");
  if($upd_completed)
  {
...
if($btn_action=="Action"&&$action=="cancelled")
{
  ...
  $upd_cancelled = $conn->Execute("UPDATE ".PRODUCTTRANSFER." SET status='N' WHERE id IN(".$selData.")");
  if($upd_cancelled)
  {
...
{
	$selData = @implode(",",$chkall);
	$upd_inactive = $conn->Execute("UPDATE ".PRODUCTTRANSFER." SET status='D' WHERE id IN(".$selData.")",1);
...
$selTable = $conn->pagination(PRODUCTTRANSFER,"*",$cond,"id desc",$website,$paging,$_GET['page']);
              ...
              <div class="box-body table-responsive no-padding">
                ...
                <table class="table table-hover">
                  <tr class="bg-gray color-palette">
                    ...
                    <th class="head_text" width="25%">Number</th>
               ...
               $users = $conn->select_query(USER,"*","user_id ='".$res['user_id']."'","1");
                ...
                $order_status = $conn->select_query(PRODUCTTRANSFER,"*","status='".$res['status']."' AND status='W'","1");
								$sino++;
						   ?>
                  <tr class="tbldatarow">
                    ...
                    <td class="td_text"><?php echo $conn->stripval(ucfirst($users['user_name']));?></td>
                        <td class="td_text"><?php echo $conn->stripval($res['number']);?></td>
