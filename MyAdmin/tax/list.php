...
if($btn_action=="Action"&&$action=="Active")
{
	...
	$upd_inactive = $conn->Execute("UPDATE ".TAX." SET tax_status='Y' WHERE tax_id IN(".$selData.")");
	if($upd_inactive)
	{
...
#Select Records
$cond = ($token=="inactive") ? "tax_status='N'" : "tax_status='Y'";
if($txtsearch)
{
...
$selTable = $conn->pagination(TAX,"*",$cond,"tax_pos asc",$website,$paging,$_GET['page']);
?>
<?php #Admin Html head
$conn->adminHtmlhead();
$conn->admninBody();
              ...
              <div class="box-body table-responsive no-padding">
                ...
                <table class="table table-hover">
                  <tr class="bg-gray color-palette">
                    ...
                    <th class="head_text" width="25%">Tax Percentage</th>
                  ...
                  <tr class="tbldatarow">
                    ...
                    <td class="td_text"><?php echo $conn->stripval(ucfirst($res['tax_name']));?></td>
                    <td class="td_text"><?php echo $res['tax_percentage'].' %'; ?></td>
                    <td class="td_text"><?php echo $res['tax_pos'] ?></td>
                    <td class="td_text"><a class="btn btn-default" href="<?php echo $path_folder."form.php"?>?q=<?php echo $res['tax_id'].'&rpage='.base64_encode($conn->getOwnURL());?>"  title="Edit"><i class="fa fa-edit"></i></a></td>
					<td class="td_text"><a class="btn btn-default" href="<?php echo $path_folder."view.php"?>?q=<?php echo $res['tax_id'];?>"  title="View"><i class="fa fa-file-text"></i></a></td>
				<td class="td_text"><input name="chkall[]" id="chkall" class="chkall" type="checkbox" value="<?php echo $res['tax_id']; ?>" /></td>
                  </tr>
                  <?php }?>
                </table>
                <?php }else{?>
