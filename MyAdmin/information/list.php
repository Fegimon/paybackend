...
if($btn_action=="Action"&&$action=="Inactive")
{
	...
	$upd_inactive = $conn->Execute("UPDATE ".Information." SET information_status='N' WHERE id IN(".$selData.")");
	if($upd_inactive)
	{
...
if($btn_action=="Action"&&$action=="Active")
{
	...
	$upd_inactive = $conn->Execute("UPDATE ".Information." SET information_status='Y' WHERE id IN(".$selData.")");
	if($upd_inactive)
	{
...
{
	$selData = @implode(",",$chkall);
	$upd_inactive = $conn->Execute("UPDATE ".Information." SET information_status='D' WHERE id IN(".$selData.")",1);
...
#Select Records
$cond = ($token=="inactive") ? "information_status='N'" : "information_status='Y'";
if($txtsearch)
{
	$cond .= " AND (name LIKE '%".base64_decode($txtsearch)."%' OR email LIKE '%".base64_decode($txtsearch)."%')";
...
$selTable = $conn->pagination(INFORMATION,"*",$cond,"id asc",$website,$paging,$_GET['page']);
              ...
              <div class="box-body table-responsive no-padding">
                ...
                <table class="table table-hover">
                  <tr class="bg-gray color-palette">
                    ...
                    <th class="head_text" width="25%">Email</th>
                    <th class="head_text" width="15%">Phone</th>
                  ...
                  <tr class="tbldatarow">
                    ...
                    <td class="td_text"><?php echo $conn->stripval(ucfirst($res['name']));?></td>
                    <td class="td_text"><?php echo $res['email'].' %'; ?></td>
                    <td class="td_text"><?php echo $res['phone'] ?></td>
                    <td class="td_text"><a class="btn btn-default" href="<?php echo $path_folder."form.php"?>?q=<?php echo $res['id'].'&rpage='.base64_encode($conn->getOwnURL());?>"  title="Edit"><i class="fa fa-edit"></i></a></td>
				...
				<td class="td_text"><input name="chkall[]" id="chkall" class="chkall" type="checkbox" value="<?php echo $res['id']; ?>" /></td>
                  </tr>
                  <?php }?>
                </table>
                <?php }else{?>
