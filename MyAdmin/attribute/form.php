...
$id = $conn->variable($q);
$sel = $conn->select_query(ATTRIBUTE,"*","at_id='".$id."'","1");
if(!$sel['nr'] && !empty($id))
{
...
if($id){
  ...
  if(isset($edit))
  {
      $new = array('at_updated'=>NOW);
      $ins = $conn->update(ATTRIBUTE,"at_id='".$id."'",$new);
      if($ins)
      {
  ...
  if(isset($add))
{
    ...
    $new = array('at_status'=>'Y','at_dt'=>NOW);
    $ins = $conn->insert(ATTRIBUTE,"",$new);
    if($ins)
    {
                ...
                <div class="box-body">
                  <form role="form" method="post" name="frm_new" id="frm_new" action="" enctype="multipart/form-data">
					...
					<div class="form-group">
                      ...
                      <input  name="at_name" id="at_name" type="text" class="form-control validate[required]" placeholder="Enter Name" maxlength="200" value="<?php echo $conn->stripval($sel['at_name']);?>" />
                    </div>
                    <div class="form-group">
                      <label>Position <span class="text-red">*</span></label>
                      <input name="at_pos" id="at_pos" type="text" class="form-control validate[required,custom[integer]] " placeholder="Enter Position"  maxlength="2"  style="width:110px;" value="<?php echo $conn->stripval($sel['at_pos']);?>" />
                      </div>
