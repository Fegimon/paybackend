...
if($btn_action=="Action"&&$action=="Inactive")
{
	...
	$upd_inactive = $conn->Execute("UPDATE ".ATTRIBUTE." SET at_status='N' WHERE at_id IN(".$selData.")");
	if($upd_inactive)
	{
...
if($btn_action=="Action"&&$action=="Active")
{
	...
	$upd_inactive = $conn->Execute("UPDATE ".ATTRIBUTE." SET at_status='Y' WHERE at_id IN(".$selData.")");
	if($upd_inactive)
	{
...
{
	$selData = @implode(",",$chkall);
	$upd_inactive = $conn->Execute("UPDATE ".ATTRIBUTE." SET at_status='D' WHERE at_id IN(".$selData.")",1);
...
#Select Records
$cond = ($token=="inactive") ? "at_status='N'" : "at_status='Y'";
if($txtsearch)
{
	$cond .= " AND (at_name LIKE '%".base64_decode($txtsearch)."%' OR at_status LIKE '%".base64_decode($txtsearch)."%')";
...
$selTable = $conn->pagination(ATTRIBUTE,"*",$cond,"at_pos asc",$website,$paging,$_GET['page']);
?>
<?php #Admin Html head
$conn->adminHtmlhead();
$conn->admninBody();
              ...
              <div class="box-body table-responsive no-padding">
                ...
                <table class="table table-hover">
                  <tr class="bg-gray color-palette">
                    ...
                    <th class="head_text" width="25%">Date Added</th>
                  ...
                  <tr class="tbldatarow">
                    ...
                    <td class="td_text"><?php echo $conn->stripval(ucfirst($res['at_name']));?></td>
                    <td class="td_text"><?php echo $res['at_pos'] ?></td>
                    <td class="td_text"><?php echo date("d-m-Y",strtotime($res['at_dt']));?></td>
                    <td class="td_text"><a class="btn btn-default" href="<?php echo $path_folder."form.php"?>?q=<?php echo $res['at_id'].'&rpage='.base64_encode($conn->getOwnURL());?>"  title="Edit"><i class="fa fa-edit"></i></a></td>
					<!--td class="td_text"><a class="btn btn-default" href="<?php echo $path_folder."view.php"?>?q=<?php echo $res['at_id'];?>"  title="View"><i class="fa fa-file-text"></i></a></td-->
				<td class="td_text"><input name="chkall[]" id="chkall" class="chkall" type="checkbox" value="<?php echo $res['at_id']; ?>" /></td>
                  </tr>
                  <?php }?>
                </table>
                <?php }else{?>
