...
if($btn_action=="Action"&&$action=="Inactive")
{
	...
	$upd_inactive = $conn->Execute("UPDATE ".COMMONSERVICE." SET brand_status='N' WHERE id IN(".$selData.")");
...
if($btn_action=="Action"&&$action=="Active")
{
	...
	$upd_inactive = $conn->Execute("UPDATE ".COMMONSERVICE." SET brand_status='Y' WHERE id IN(".$selData.")");
	if($upd_inactive)
	{
...
{
	$selData = @implode(",",$chkall);
	$upd_inactive = $conn->Execute("UPDATE ".COMMONSERVICE." SET brand_status='D',brand_img='' WHERE id IN(".$selData.")",1);
	if($upd_inactive)
	{
		#DELETE IMAGE
		$delDat = $conn->select_query(COMMONSERVICE,"brand_img","id IN(".$selData.")","");
		if($delDat['nr'])
		{
...
if($btn_action=="Action"&&$action=="Position")
{
	if(count($chkall)>0)
	{
		...
		{
			$updatepos = $conn->Execute("UPDATE ".COMMONSERVICE." SET brand_pos='".$position[$id]."' WHERE id='".$id."'");		}
	}
...
$selTable = $conn->pagination(COMMONSERVICE,"*",$cond,"",$website,$paging,$_GET['page']); 
?>
<?php #Admin Html head
$conn->adminHtmlhead();
$conn->admninBody();
