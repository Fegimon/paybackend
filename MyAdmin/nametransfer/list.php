...
if($btn_action=="Action"&&$action=="processing")
{
  ...
  //print_r($selData);
	...
	$upd_processing = $conn->Execute("UPDATE ".NAMETRANSFER." SET status='Y' WHERE name_trans_id IN(".$selData.")");
	if($upd_processing)
	{
...
if($btn_action=="Action"&&$action=="completed")
{
  ...
  $upd_completed = $conn->Execute("UPDATE ".NAMETRANSFER." SET status='C' WHERE name_trans_id IN(".$selData.")");
  if($upd_completed)
  {
...
if($btn_action=="Action"&&$action=="cancelled")
{
  ...
  $upd_cancelled = $conn->Execute("UPDATE ".NAMETRANSFER." SET status='N' WHERE name_trans_id IN(".$selData.")");
  if($upd_cancelled)
  {
...
{
	$selData = @implode(",",$chkall);
	$upd_inactive = $conn->Execute("UPDATE ".NAMETRANSFER." SET status='D' WHERE name_trans_id IN(".$selData.")",1);
...
$selTable = $conn->pagination(NAMETRANSFER,"*",$cond,"name_trans_id desc",$website,$paging,$_GET['page']);
    ...
    <section class="content">
              ...
              <div class="box-body table-responsive no-padding">
                ...
                <table class="table table-hover">
                  <tr class="bg-gray color-palette">
                    ...
                    <th class="head_text" width="25%">Product Name</th>
                ...
                $order_status = $conn->select_query(SERVICEREQUEST,"*","status='".$res['status']."' AND status='W'","1");
     ...
     $users = $conn->select_query(USER,"*","user_id ='".$res['user_id']."'","1");
                  ...
                  <tr class="tbldatarow">
                    ...
                    <td class="td_text"><?php echo $res['prod_name']; ?></td>
                   ...
                   <?php /* ?> <td class="td_text"><a class="btn btn-default" href="<?php echo $path_folder."form.php"?>?q=<?php echo $res['tax_id'].'&rpage='.base64_encode($conn->getOwnURL());?>"  title="Edit"><i class="fa fa-edit"></i></a></td><?php */ ?>
					...
					<td class="td_text"><a class="btn btn-default" href="<?php echo $path_folder."view.php"?>?q=<?php echo $res['name_trans_id'];?>"  title="View"><i class="fa fa-file-text"></i></a></td>
				<?php  ?><td class="td_text"><input name="chkall[]" id="chkall" class="chkall" type="checkbox" value="<?php echo $res['name_trans_id']; ?>" /></td><?php  ?>
                  </tr>
                  <?php }?>
                </table>
                <?php }else{?>
