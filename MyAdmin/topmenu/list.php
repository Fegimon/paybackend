...
if($btn_action=="Action"&&$action=="Inactive")
{
	...
	$upd_inactive = $conn->Execute("UPDATE ".TOPMENU." SET tl_status='N' WHERE tl_id IN(".$selData.")");
	if($upd_inactive)
	{
...
if($btn_action=="Action"&&$action=="Active")
{
	...
	$upd_inactive = $conn->Execute("UPDATE ".TOPMENU." SET tl_status='Y' WHERE tl_id IN(".$selData.")");
	if($upd_inactive)
	{
...
{
	$selData = @implode(",",$chkall);
	$upd_inactive = $conn->Execute("UPDATE ".TOPMENU." SET tl_status='D',post_image='' WHERE tl_id IN(".$selData.")",1);
	if($upd_inactive)
	{
		#DELETE IMAGE
...
$selTable = $conn->pagination(TOPMENU,"*",$cond,"",$website,$paging,$_GET['page']);
?>
<?php #Admin Html head
$conn->adminHtmlhead();
$conn->admninBody();
