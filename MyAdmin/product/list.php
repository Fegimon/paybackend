...
if($btn_action=="Action"&&$action=="Position")
{
	if(count($chkall)>0)
	{
		...
		{
			$updatepos = $conn->Execute("UPDATE ".PRODUCT." SET p_position='".$position[$id]."' WHERE p_id='".$id."'");		}
	}
...
$selTable = $conn->pagination(PRODUCT,"*",$cond,"p_position asc",$website,$paging,$_GET['page'],1);
                ...
                <table class="table table-hover">
                  <tr class="bg-gray color-palette">
                    <th class="head_text" width="6%">#</th>
					          <th class="head_text" width="16%">Product Name</th>
                    <th class="head_text" width="16%">Slug</th>
                   <!-- <th class="head_text" width="15%">Description</th>-->
                    <th class="head_text" width="12%">Date Added</th>
                    <!--th class="head_text" width="8%">Comments</th>
                    <th class="head_text" width="15%">Image</th-->
                    <th class="head_text" width="15%">Position</th>
                    <th class="head_text" width="14%">Edit</th>
                    <th class="head_text" width="8%">View</th>
                    <th class="head_text" width="13%">Select</th>
                  </tr>
                  <?php 
...
<!--td align="center" class="td_text"><a href="<?php echo $path_folder;?>view_post_approved_comments.php?id=<?php echo base64_encode($res['post_id']);?>&prod_name=<?php echo base64_encode($res['post_title']);?>" style="color:#006600; text-decoration:none;"><?php echo "(".$commandcounts['actcount'].")"; ?></a> | <a href="<?php echo $path_folder;?>view_post_unapproved_comments.php?id=<?php echo base64_encode($res['post_id']);?>&prod_name=<?php echo base64_encode($res['post_title']);?>" style="color:#FF0000; text-decoration:none;"><?php echo "(".$commandcounts['disactcount'].")"; ?></a></td-->
          <?php /*?><td class="td_text"><?php echo $conn->stripval($res['p_desc']);?></td><?php */?>
          <td class="td_text"><?php echo date("d-m-Y",strtotime($res['p_date_dt']));?></td>
          
           <td class="td_text"><input type="text" name="position[<?php echo $res['p_id']; ?>]" style="width:60px; text-align:center;" id="position<?php echo $res['p_id']; ?>" value="<?php echo $res['p_position']; ?>" maxlength="2" class="validate[required,custom[integer]] form-control"  /></td>
