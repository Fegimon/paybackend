...
if($btn_action=="Action"&&$action=="processing")
{
  ...
  //print_r($selData);
	...
	$upd_processing = $conn->Execute("UPDATE ".COMMONSERVICEREQUEST." SET status='N' WHERE id IN(".$selData.")");
	if($upd_processing)
	{
...
if($btn_action=="Action"&&$action=="completed")
{
  ...
  $upd_completed = $conn->Execute("UPDATE ".COMMONSERVICEREQUEST." SET status='D' WHERE id IN(".$selData.")");
  if($upd_completed)
  {
...
if($btn_action=="Action"&&$action=="cancelled")
{
  ...
  $upd_cancelled = $conn->Execute("UPDATE ".COMMONSERVICEREQUEST." SET status='C' WHERE id IN(".$selData.")");
  if($upd_cancelled)
  {
...
{
	$selData = @implode(",",$chkall);
	$upd_inactive = $conn->Execute("UPDATE ".COMMONSERVICEREQUEST." SET status='D' WHERE id IN(".$selData.")",1);
...
switch($token)
{
  ...
  case 'processing':$cond= "status='N'";
  $ttxt="processing";
  $website_cond="processing";
  $orderby="id asc";
  break;
  
  case 'completed':$cond= "status='D'";
  $ttxt="completed";
...
$selTable = $conn->pagination(COMMONSERVICEREQUEST,"*",$cond,"id desc",$website,$paging,$_GET['page']);
          ...
          <div class="box <?php echo ($token=="inactive")?"box-danger":"box-success"; ?>">
            ...
            <form method="post" name="frm_list" id="frm_list" >
              <div class="box-body table-responsive no-padding">
                ...
                $order_status = $conn->select_query(COMMONSERVICEREQUEST,"*","status='".$res['status']."' AND status='W'","1");
								$sino++;
						   ?>
                  <tr class="tbldatarow">
                    <td class="td_text"><?php echo $sino; ?></td>
                    <td class="td_text"><?php echo $conn->stripval($res['id']);?></td>
                    <td class="td_text"><?php echo $res['name']; ?></td>
                    <td class="td_text"><?php echo $res['description']; ?></td>
