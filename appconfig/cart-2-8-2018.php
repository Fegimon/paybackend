...
{    
    ...
    public function getProducts(){
    	...
    	if($cart_query['nr']){
    		foreach ($cart_query['result'] as $cart) {
				...
				if($product_query['p_category']!='3')
				{
					
					if($cart['option_cart']=='1'){
					$prodprice=$product_query['pp_price_3_month'];
					}elseif($cart['option_cart']=='2'){
					$prodprice=$product_query['pp_price_6_month'];
					}elseif($cart['option_cart']=='3'){
					$prodprice=$product_query['pp_price_9_month'];
					}elseif($cart['option_cart']=='12'){
					$prodprice=$product_query['pp_price_12_month'];
					
					}
				}else
				{
					$prodprice=$product_query['ps_price_month'];
				}
				
    			if ($product_query['nr'] && ($cart['quantity'] > 0)) {
    				//echo "<pre>"; print_r($product_query); exit;
                    $tax_amount = ($prodprice) * ($product_query['tax_percentage']/100);
                    $price = $prodprice + $tax_amount;
                   // echo $price;
                    $product_data[] = array(
				   ...
				   'rent_days'        => $cart['rent_days'],
                    ...
                    'price_month'       => $prodprice,
					'p_category'       => $product_query['p_category'],
    ...
    public function addCart($product_id, $quantity = 1, $option,$days) {
	  ...
	  /* if($cart_chk)
       ...
       // print_r($query); 
        ...
        if (!$query['total']) {
           // echo "if"; exit;
            $new = array( 'customer_id' => (int)$_SESSION['prentz_user_id'], 'session_id' => session_id(), 'product_id' => (int)$product_id, 'quantity'=>$quantity, 'option_cart'=>$option, 'date_added'=>NOW,'rent_days'=>$days);
