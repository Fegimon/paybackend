...
#inpFolder {
    ...
    <script language="javascript" type="text/javascript">
		...
		$content = @str_replace('\\','/',$dirname);
		$dirroot=@str_replace($_SERVER['DOCUMENT_ROOT'],'',$content);
		?>
	
        var base = "<?php echo $dirroot; ?>/assests"; /*Relative to Root*/
        var readonly = false;
        var fullpath = true;
        ...
        function renderTree() {
            ...
            $('#container_id').fileTree({
                ...
                if ($.inArray(ext, ['gif', 'png', 'jpg', 'jpeg']) != -1) {
                    $("#preview_id").html("<img id='imgFile' src='" + file + "' style='width:70px;padding:4px;border:#cccccc 1px solid;background:#ffffff;margin-bottom:3px' /><br /><a id='lnkDelFile' style='position:absolute;top:-5px;right:-5px' href='javascript:deleteFile()'><img src='images/delete.png' style='border:none' /></a><a id='idFile' href='" + file + "' target='_blank' style='background:#ffffff;color:#000000;margin:2px;'>" + filename + "</a>");
                    ...
                    if (ext.indexOf("/") == -1) {
                        $("#preview_id").html("<a id='lnkDelFile' style='float:right' href='javascript:deleteFile()'><img src='images/delete.png' style='border:none;' /></a><a id='idFile' target='_blank' href='" + file + "' style='color:#000000;background:#ffffff;margin-right:5px;'>" + filename + '</a>');
                        if (fullpath) { fileurl = window.location.protocol + "//" + window.location.host + file }
                        else { fileurl = file };
                        try {
							parent.fileclick(fileurl);
    ...
    <form id="form1">
      <div style="padding:15px;background:#fcfcfc;border-bottom:#f7f7f7 1px solid">
        <input id="active_folder" type="hidden" />
        <div id="folder_id" style="font-weight:bold;font-size:14pt">/</div>
        <div style="margin-top:12px;margin-bottom:7px;"> <a id="lnkNewFolder" href="javascript:panelFolder()" style="margin-right:10px;font-size:10px;color:#000;">NEW FOLDER</a> <a id="lnkUpload" href="javascript:upload()" style="margin-right:10px;font-size:10px;color:#000;">UPLOAD</a> <a href="javascript:preview()" style="margin-right:10px;font-size:10px;color:#000;">PREVIEW</a> </div>
        <div id="divUpload" style="padding:17px;border:#f3f3f3 1px solid;background:#fefefe;"> </div>
        <div id="divNewFolder" style="padding:17px;height:65px;border:#f3f3f3 1px solid;background:#fefefe;">
        ...
        <div id="divPreview" style="position:relative;padding:17px;border:#f3f3f3 1px solid;height:65px;background:#fefefe;">
          <div id="preview_id" style="position:absolute;left:20px;top:15px;width:80px;height:80px;text-align:right;"> </div>
        </div>
      </div>
